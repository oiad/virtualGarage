...
*/
	...
	*/
	...
	if ((_typeOfCursorTarget in vg_List) && (player distance _cursorTarget < 5)) then {
		_hasAccess = [player,_cursorTarget] call FNC_check_access;
		if (s_garage_dialog < 0 && ((_hasAccess select 0) or (_hasAccess select 2) or (_hasAccess select 3) or (_hasAccess select 4) or (!isPlayer _cursorTarget && {_typeOfCursorTarget in vg_list}))) then {
